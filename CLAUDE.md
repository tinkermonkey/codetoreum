...
**Ports** — input + output interfaces in `ports/`. Output ports include: ITicketSystem, ICodingAgent, IContainer, IVersionControlService, IEventStore, IBoardService, ICodeReviewService, IWorkItemService, IEventEmitter, IIdentityService, IPipelineLockService, IPromptBuilder, IDistributedLock. (42 total output ports — see `documentation/architecture/ports/README.md` for the full catalogue.)
