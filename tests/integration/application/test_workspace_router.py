...
# ============================================================================
# Tests: Credential Validation in Environment Variables
# ============================================================================


@pytest.mark.asyncio
async def test_prepare_container_environment_rejects_aws_credentials(
    workspace_router, sample_work_item, sample_agent, sample_project
):
    """Test that AWS_* prefix patterns are blocked."""
    # Arrange
    project_with_creds = ProjectContext(
        id=sample_project.id,
        name=sample_project.name,
        display_name=sample_project.display_name,
        repository_url=sample_project.repository_url,
        default_branch=sample_project.default_branch,
        branch_prefix=sample_project.branch_prefix,
        tech_stack=sample_project.tech_stack,
        primary_language=sample_project.primary_language,
        test_command=sample_project.test_command,
        test_framework=sample_project.test_framework,
        has_ci_cd=sample_project.has_ci_cd,
        default_workflow_template_id=sample_project.default_workflow_template_id,
        custom_workflows=sample_project.custom_workflows,
        has_dockerfile=sample_project.has_dockerfile,
        dockerfile_path=sample_project.dockerfile_path,
        requires_dev_container=sample_project.requires_dev_container,
        environment_variables={
            "AWS_ACCESS_KEY_ID": "AKIAIOSFODNN7EXAMPLE",
        },
        secrets=sample_project.secrets,
        mcp_servers=sample_project.mcp_servers,
    ...
    with pytest.raises(ValidationError) as exc_info:
        await workspace_router.prepare_container_environment(
            context=context, project=project_with_creds, agent=sample_agent
        )
...
@pytest.mark.asyncio
async def test_prepare_container_environment_rejects_github_token(
    workspace_router, sample_work_item, sample_agent, sample_project
):
    """Test that GITHUB_* prefix patterns are blocked."""
    # Arrange
    project_with_creds = ProjectContext(
        id=sample_project.id,
        name=sample_project.name,
...
@pytest.mark.asyncio
async def test_prepare_container_environment_rejects_password_prefix(
    workspace_router, sample_work_item, sample_agent, sample_project
):
    """Test that PASSWORD_* prefix patterns are blocked (not substring)."""
    # Arrange
    project_with_creds = ProjectContext(
        id=sample_project.id,
        name=sample_project.name,
...
@pytest.mark.asyncio
async def test_prepare_container_environment_allows_legitimate_token_variable(
    workspace_router, sample_work_item, sample_agent, sample_project
):
    """Test that TOKEN_ prefix only blocks variables starting with TOKEN_, not containing 'TOKEN'."""
    # Arrange - Variables like AUTH_TOKEN_EXPIRY, MAX_TOKEN_LENGTH should be allowed
    project_with_vars = ProjectContext(
        id=sample_project.id,
        name=sample_project.name,
        display_name=sample_project.display_name,
        repository_url=sample_project.repository_url,
    ...
    context = await workspace_router.route_workspace(
        work_item=sample_work_item,
        agent=sample_agent,
        project=project_with_vars,
    )

    # Act - Should not raise ValidationError
    env_vars = await workspace_router.prepare_container_environment(
        context=context, project=project_with_vars, agent=sample_agent
    )

    # Assert - All variables should be present
    assert "AUTH_TOKEN_EXPIRY" in env_vars
    assert "MAX_TOKEN_LENGTH" in env_vars
    assert "CACHE_TOKENS" in env_vars


@pytest.mark.asyncio
async def test_prepare_container_environment_allows_legitimate_password_variable(
    workspace_router, sample_work_item, sample_agent, sample_project
):
    """Test that PASSWORD_ prefix only blocks PASSWORD_* not variables containing PASSWORD."""
    # Arrange
    project_with_vars = ProjectContext(
        id=sample_project.id,
        name=sample_project.name,
    ...
    # Assert - All variables should be present
    assert "RESET_PASSWORD_URL" in env_vars
    assert "REQUIRE_PASSWORD_CHANGE" in env_vars


@pytest.mark.asyncio
async def test_prepare_container_environment_rejects_token_prefix(
    workspace_router, sample_work_item, sample_agent, sample_project
):
    """Test that TOKEN_* prefix patterns are blocked."""
    # Arrange
    project_with_creds = ProjectContext(
        id=sample_project.id,
        name=sample_project.name,
        ...
        requires_dev_container=sample_project.requires_dev_container,
        environment_variables={
            "TOKEN_SECRET": "super_secret_token_value",
        },
        secrets=sample_project.secrets,
        mcp_servers=sample_project.mcp_servers,
        metadata=sample_project.metadata,
    ...
    assert "Forbidden credential pattern" in str(exc_info.value)
    assert "TOKEN_SECRET" in str(exc_info.value)


@pytest.mark.asyncio
async def test_prepare_container_environment_allows_safe_variables(
    workspace_router, sample_work_item, sample_agent, sample_project
):
    """Test that legitimate project environment variables are allowed."""
    # Arrange - Create project with safe environment variables
    project_with_vars = ProjectContext(
        id=sample_project.id,
        name=sample_project.name,
        display_name=sample_project.display_name,
    ...
    assert "PROJECT_KEY" in env_vars
    assert "DEBUG" in env_vars
    assert "LOG_LEVEL" in env_vars
    assert "MAX_WORKERS" in env_vars
    assert env_vars["ENV"] == "test"
    assert env_vars["PROJECT_KEY"] == "test-key"
