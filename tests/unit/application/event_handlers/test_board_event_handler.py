...
class TestPRReviewCycleDispatch:
    ...
    @pytest.mark.asyncio
    async def test_outcome_events_include_next_column(
        self,
        handler,
        mock_workflow_config,
    ):
        ...
        # Mock the cycle to return a completed state
        completed_state = PRReviewCycleStateData(
            work_item_id="item-1",
            project_id="proj-1",
            board_id="board-1",
            cycle_number=1,
        ...
        pr_review_cycle_mock.get_cycle_state.return_value = None
        pr_review_cycle_mock.start_pr_review_cycle.return_value = completed_state
        ...
        # Assert - start_pr_review_cycle was called
        pr_review_cycle_mock.start_pr_review_cycle.assert_called_once()

        # Verify the design note: outcome events are responsible for column movement
        # The test verifies that the cycle is dispatched, and column movement
        # is expected to be driven by PRReviewCycleApprovedEvent or PRReviewCycleIssuesFoundEvent
