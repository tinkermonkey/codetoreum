...
class TestLegacyWorkItemColumnChangedEventHandling:
    """Tests for legacy WorkItemColumnChanged event support in PRReviewCycleDispatchHandler."""

    async def test_handles_legacy_event_with_valid_payload(self):
    ...
    async def test_raises_on_legacy_event_missing_work_item_id(self, caplog):
        ...
        # Create legacy event missing work_item_id
        legacy_event = WorkItemColumnChanged(
            aggregate_id="legacy-id",
            payload={
                "board_id": "board-1",
