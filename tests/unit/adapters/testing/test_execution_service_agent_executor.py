...
class ExecutorFixture:
    ...
    async def drain_pending(self, executor: ExecutionServiceAgentExecutor, deadline_s: float = 1.0) -> None:
        ...
        cascade scheduled by the publish's on_error callback. Tests that assert
        against the completion bridge must wait for both. We drain the bus, then
        the executor's tasks, and repeat until neither has work left (a publish
        failure can schedule a recovery task, which is executor-tracked).
        """
        ...
        while _inflight() and loop_count < 20:
            # Bus-tracked detached publishes (completion event delivery).
            await self.event_bus.drain(timeout=deadline_s)
            # Executor-tracked tasks (outer run task, recovery cascades).
