...
class TestListIssues:
    ...
    def test_list_after_create(self, client):
        client.post("/api/v2/simulation/ticketing/issues", json={
            "title": "Issue 1",
            "project_id": "proj-1",
        })
        client.post("/api/v2/simulation/ticketing/issues", json={
...
class TestGetIssue:
    ...
    def test_get_existing_issue(self, client):
        create_resp = client.post("/api/v2/simulation/ticketing/issues", json={
            "title": "Get me",
            "project_id": "proj-1",
        })
...
class TestMoveIssue:
    ...
    def test_move_nonexistent_issue(self, client):
        resp = client.post("/api/v2/simulation/ticketing/issues/nonexistent/move", json={
            "target_column": "Ready",
        })
        assert resp.status_code == 404
...
class TestAddComment:
    ...
    def test_add_comment_nonexistent_issue(self, client):
        resp = client.post("/api/v2/simulation/ticketing/issues/nonexistent/comment", json={
            "body": "Orphan comment",
        })
        assert resp.status_code == 404
...
class TestBoardColumns:
    ...
    def test_get_columns_nonexistent_board(self, client):
        resp = client.get("/api/v2/simulation/ticketing/board/nonexistent/columns", params={
            "project_id": "proj-1",
        })
        assert resp.status_code == 404
...
class TestBoardHistory:
    ...
    def test_history_after_move(self, client):
        ...
        # Move
        client.post(f"/api/v2/simulation/ticketing/issues/{issue_id}/move", json={
            "target_column": "Ready",
        })
