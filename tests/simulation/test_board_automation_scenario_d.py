...
class TestScenarioD_HumanIntervention:
    ...
    async def test_human_backward_movement(self, setup):
        ...
        assert agent_executor.is_repair_cycle_active(
            "work-item-100"
        ...
        assert engineer_count >= 2, (
            f"Engineer should be triggered twice (initial + re-trigger after human movement), "
    ...
    async def test_human_movement_always_respected(self, setup):
        ...
        human_movements = [
            h for h in history if h.moved_by == MovedByType.HUMAN
        ]
    ...
    async def test_human_skip_forward(self, setup):
        ...
        # Step 1: Human manually moves directly to Staged (skips Code Review and Testing)
        await board_service.move_item_to_column(
