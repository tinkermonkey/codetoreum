...
class WorkItemUpdatedEvent(CodetoreumEvent):
    ...
    Note: The 'changes' dict is a mutable reference to support serialization.
    This is a known limitation - ideally it should be a frozendict or tuple
    of tuples. Current workaround: treat changes as read-only after creation.
    """
    ...
    def __post_init__(self) -> None:
        ...
        if not self.project_id:
            raise ValueError("project_id is required")
        # Ensure changes is initialized to empty dict if None
