...
class ServiceResilienceConfig:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if self.timeout:
            result["timeout"] = {"default_timeout_seconds": self.timeout.default_timeout_seconds}
    ...
    def from_dict(cls, data: dict[str, Any]) -> "ServiceResilienceConfig":
        ...
        if "timeout" in data:
            t = data["timeout"]
            timeout = TimeoutConfig(default_timeout_seconds=t.get("default_timeout_seconds", 30.0))
...
GITHUB_RESILIENCE_CONFIG = ServiceResilienceConfig(
    ...
    circuit_breaker=CircuitBreakerConfig(failure_threshold=5, timeout_seconds=60, success_threshold=2),
    retry=RetryConfig(max_retries=3, base_delay=1.0, max_delay=30.0),
    timeout=TimeoutConfig(default_timeout_seconds=30.0),
)
...
CONTAINER_RESILIENCE_CONFIG = ServiceResilienceConfig(
    ...
    rate_limit=RateLimitConfig(max_requests=100, window_seconds=60, max_wait_seconds=30),
    circuit_breaker=CircuitBreakerConfig(failure_threshold=5, timeout_seconds=60, success_threshold=2),
    retry=RetryConfig(max_retries=3, base_delay=1.0, max_delay=30.0),
    timeout=TimeoutConfig(default_timeout_seconds=60.0),
)

# Git/Repository configuration
REPOSITORY_RESILIENCE_CONFIG = ServiceResilienceConfig(
    service_name="repository",
    rate_limit=RateLimitConfig(max_requests=200, window_seconds=60, max_wait_seconds=30),
    circuit_breaker=CircuitBreakerConfig(failure_threshold=5, timeout_seconds=60, success_threshold=2),
    retry=RetryConfig(max_retries=3, base_delay=1.0, max_delay=30.0),
    timeout=TimeoutConfig(default_timeout_seconds=45.0),
)
