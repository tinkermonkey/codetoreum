...
Data format: The unit of configuration is BoardWorkflowTemplate, which is keyed by
...
class IWorkflowConfigService(ABC):
    ...
    Example — runtime read: template = await service.get_board_workflow_template("board-123")
        if template: col = template.get_column_config("In Progress")
    ...
    async def get_board_workflow_template(self, board_id: str) -> BoardWorkflowTemplate | None:
        ...
        Args: board_id: Board identifier (matches template.board_id)

        Returns: BoardWorkflowTemplate if a template has been configured for this
            board, None if the board has no template.

        Raises: ExternalServiceError: On backing-store communication failure.
        """
    ...
    async def save_board_workflow_template(self, template: BoardWorkflowTemplate) -> None:
        ...
        Args: template: Fully-populated BoardWorkflowTemplate.  Both
    ...
    async def list_board_workflow_templates(self, project_id: str) -> list[BoardWorkflowTemplate]:
        ...
        Args: project_id: Project identifier (matches template.project_id)

        Returns: List of BoardWorkflowTemplate instances for the project, ordered
    ...
    async def delete_board_workflow_template(self, board_id: str) -> None:
        ...
        Args: board_id: Board identifier whose template should be deleted.
