...
class IWorkflowOrchestrator(ABC):
    ...
    async def orchestrate_project(self, project_name: str, workspace_path: str, config: ProjectConfig) -> int:
        ...
        Coordinates workflow activities for the project: 1. Checks if project is enabled via config.enabled flag
        ...
        Returns: int: Number of actions taken (tasks queued, etc.)

        Raises: ExternalServiceError: External service communication failure
        """
