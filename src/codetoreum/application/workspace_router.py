...
from codetoreum.ports.exceptions import ExternalServiceError, ValidationError
...
class WorkspaceRouter:
    ...
    async def prepare_container_environment(
        ...
        **Expected Call Order**: This method should be called AFTER prepare_workspace()
        ...
        forbidden_patterns = [
            # Cloud provider credentials (prefix patterns)
            "AWS_", "AZURE_", "GCP_", "DO_",
            # VCS tokens (prefix patterns)
            "GITHUB_", "GITLAB_", "BITBUCKET_", "GITEA_",
            # SSH/Private keys (prefix patterns)
            "SSH_", "PRIVATE_", "RSA_KEY", "KEY_",
            # Passwords and secrets (prefix patterns)
            "PASSWORD_", "PASSWD_", "TOKEN_", "SECRET_", "API_KEY",
            # Docker/Registry (prefix patterns)
            "DOCKER_", "REGISTRY_",
            # Kubernetes (prefix patterns)
            "KUBE_", "K8S_",
        ...
        for key in project.environment_variables.keys():
            ...
            for pattern in forbidden_patterns:
                if key_upper.startswith(pattern):
                    error_msg = (
