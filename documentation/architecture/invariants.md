...
Exceptions:
- The `/health` endpoint is unauthenticated (external monitoring infrastructure like load balancers requires credential-free liveness checks).
- GitHub webhook endpoints use HMAC-SHA256 signature verification instead of bearer tokens (external event source cannot carry the app's bearer token).
- The `/metrics` endpoint is unauthenticated (external Prometheus servers require credential-free scraping of Prometheus text exposition format; network-level access control is the operator's responsibility, identical posture to `/health`).
